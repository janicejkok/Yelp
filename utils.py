...
def key_of_min_value(d):
    ...
    return [x for x,y in d.items() if y == min(d.values())][0] ### list comp returns a list of the key, in order to get the number, i did [list][0] to get the number
    # END Question 0
...
def enumerate(s, start=0):
    ...
    [[5, 'f'], [6, 'i'], [7, 'v'], [8, 'e']]
    """
    ### unlocker: >>> enumerate([6, 'one', 'a'], 3)[1] 
    ###               [4,"one"]
    ...
    #for i in s:
    #    x.append([start+1,i])
    return  zip(range(start,len(s)+start),s)   #start index is start, end index is length of sequence + start. the other range is s
    #return [x.append([start+1,i]) for i in s]
    # END Question 0
